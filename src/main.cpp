...
int main(int argc, char* argv[])
{
    ...
    /* Almost all interface functions return a CURLcode (curl error code).
       CURL_GLOBAL_ALL --> Initializes everything possible.
       This sets all known bits except CURL_GLOBAL_ACK_EINTR. */
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        /* Stores the content of the url response. */
        std::string response = "";
        ...
        /* Extracts the title from the http response. */
        std::string title = extract_title(response);
        /* Extracts the url from the http response. */
